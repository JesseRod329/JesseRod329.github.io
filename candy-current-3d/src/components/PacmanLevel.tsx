...
import { Mesh, InstancedMesh, Vector3 } from 'three';
...
interface PacmanLevelProps {
  level: number;
  onPelletCollected: (pellet: Pellet) => void;
  onLevelComplete: () => void;
}

const PacmanLevel: React.FC<PacmanLevelProps> = ({ 
  level, 
  onPelletCollected, 
  onLevelComplete 
}) => {
  ...
  const platforms = useMemo(() => {
    const platformCount = Math.min(3 + level, 8); // 3-8 platforms based on level
    const platforms: Platform[] = [];
    
    for (let i = 0; i < platformCount; i++) {
      const angle = (i / platformCount) * Math.PI * 2;
      const radius = 3 + i * 0.5; // Spiral outward
      const height = i * 1.5; // Rising platforms
      ...
      const pelletCount = 3 + Math.floor(Math.random() * 4); // 3-6 pellets per platform
  ...
  React.useEffect(() => {
    if (collectedPellets.size >= totalPellets && !levelComplete) {
  ...
  // Platform physics refs
  const platformRefs = useRef<{ [key: string]: any }>({});
  ...
  useFrame((state) => {
    ...
    platforms.forEach(platform => {
      platform.pellets.forEach(pellet => {
        if (!collectedPellets.has(pellet.id)) {
          // Animate pellets (floating effect)
          const mesh = document.querySelector(`[data-pellet-id="${pellet.id}"]`) as any;
          if (mesh) {
            const yOffset = Math.sin(time * 3 + pellet.id.length) * 0.1;
            // This would need to be handled through React state or refs
          }
        }
      });
    });
    ...
    <group>
      ...
      {platforms.map((platform, index) => (
        <PlatformComponent
          ...
          index={index}
          ref={(ref) => {
            if (ref) platformRefs.current[platform.id] = ref;
          }}
        />
      ))}
...
const PlatformComponent = React.forwardRef<any, PlatformComponentProps>(
  ({ platform, index }, ref) => {
    ...
    React.useImperativeHandle(ref, () => platformRef.current);
