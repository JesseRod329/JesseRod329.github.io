...
const ObjectDetector = dynamic(() => import("./ObjectDetector/ObjectDetector"), { ssr: false });
    ...
    return (
        <>
            ...
            <main className={`home`} id="main">
                ...
                {enableObjectDetection && latLong?.lat && latLong?.long && !loading && 
                 (screen === "singleplayer" || screen === "onboarding" || (screen === "multiplayer" && multiplayerState?.inGame)) && !showAnswer && (
                    <ObjectDetector
                        lat={latLong.lat}
                        lng={latLong.long}
                        onDetection={(detection) => {
                            setDetectedObjects(prev => [...prev, detection]);
                        }}
                        enabled={enableObjectDetection && !showAnswer}
                        detectedObjects={detectedObjects}
                        onRemoveDetection={(index) => {
                            setDetectedObjects(prev => prev.filter((_, i) => i !== index));
                        }}
                    />
                )}
